...
class StoreManager {
  ...
  /**
   * Get Encryption Key
   * Returns a fixed encryption key or loads from env/file
   */
  private getEncryptionKey(): string {
    // Check if an explicit key is provided via environment
    if (process.env.CHAT2API_ENCRYPTION_KEY) {
      return process.env.CHAT2API_ENCRYPTION_KEY
    }
    
    // Check if a key is stored in a secure file
    const storagePath = this.getStoragePath()
    const keyPath = join(storagePath, '.encryption_key')
  ...
  /**
   * Derive a 32-byte key from the encryption key for AES-256
   */
  private getDerivedKey(): Buffer {
    const key = this.getEncryptionKey()
    return crypto.scryptSync(key, 'chat2api-salt', 32)
  }

  /**
   * Encrypt Sensitive Data
   * @param data Data to encrypt
   * @returns Encrypted string (iv:encrypted_data)
   */
  ...
  decryptData(encryptedData: string): string {
    ...
    try {
      // Check if it's our new format: iv:encrypted:authTag
      ...
      if (parts.length === 3) {
        ...
        // Assume it might be old electron safeStorage data (base64)
        // Without Electron, we can't decrypt it. We should ideally warn the user.
        console.warn('Found legacy encrypted data which cannot be decrypted in Node.js environment')
